...
export function getPublishedQuestions(questions: Question[]): Question[] {
    ...
    const published_questions = questionsCopy.filter(
...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    const questionsCopy = [...questions];
    const full_questions = questionsCopy.filter(
...
export function sumPublishedPoints(questions: Question[]): number {
    const publishedPoints = questions
        .filter((question: Question) => question.published) // Filter published questions
        .reduce(
            (currentSum: number, question: Question) =>
                currentSum + question.points,
            0
...
export function sameType(questions: Question[]): boolean {
    const firstCheck = questions[0].type;
    return questions.every((q) => q.type === firstCheck);
}
